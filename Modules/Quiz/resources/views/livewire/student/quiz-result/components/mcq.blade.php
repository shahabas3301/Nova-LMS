        ...
        @foreach ($attemptedQuestions?->question?->options as $option)
            <li>
                ...
                <div class="am-radio {{ $className }}">
                    <input type="radio" id="{{ 'option_' .$question_number.'_' .$option?->id }}" name="price_type" value="{{ $option?->option_text }}" disabled>
                    <label for="{{ 'option_' .$question_number.'_' .$option?->id }}">{{ $option?->option_text }}</label>
                </div>
            </li>
        @endforeach
    </ul>
